...
4인 협동 × 10팟 동시 경쟁 시뮬레이션 게임. 룰은 `RULEBOOK.md`.
...
키 없이 실행하면 **로컬 모드**입니다. 같은 브라우저에서 탭을 여러 개 열고 같은 팟 코드로 들어오면 멀티플레이가 됩니다. AI 심판 키가 없으면 중립 점수로 게임이 계속됩니다.

폰·다른 기기와 붙이려면 `.env.local`에 Supabase 값을 넣으세요.
...
| 은유 카드 추가 | `data/cards.json` (개발자 없이 가능) |
...
1. **교차 감시(R3)** — 감시자 화면을 옆 팟 채널로 스위치. `useRoom`에 `pod-{다음코드}` 구독 하나 추가하면 됩니다. 15분.
2. **스틸(R4)** — 전체 답변 로그 공개 + 타 팟 복원 제출. 글로벌 채널에 `logs` 이벤트 추가. 20분.
